...
public abstract class AbstractSpreadsheet
{
    ...
    {
        visited.Add(name); //adds the named cell into the visited collection
        foreach (string n in GetDirectDependents(name))
        {
            if (n.Equals(start)) //if the named cell is within its own list of direct dependents,
            {
                throw new CircularException(); //throw an exception
            }
            else if (!visited.Contains(n)) //if the dependent has not been visited
            {
                Visit(start, n, visited, changed); //visit all of their dependents
            }
        }
        changed.AddFirst(name); //add the specified cell to the changed LinkedList
    }
